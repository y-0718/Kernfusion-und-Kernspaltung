...
export function TitleSlide({ slide, content, design }: Props) {
  ...
  const heroY = useTransform(depthProgress, [0, 1], reduceMotion ? [0, 0] : [-34, 34]);
  const heroScale = useTransform(depthProgress, [0, 0.5, 1], reduceMotion ? [1, 1, 1] : [1.08, 1.02, 1.08]);

  return (
    <SlideShell design={design} backgroundUrl={slide.background_url} className="items-center">
      {design.spaceHero ? <SolarHero y={heroY} scale={heroScale} /> : null}
...
function SolarHero({ y, scale }: { y: MotionValue<number>; scale: MotionValue<number> }) {
  return (
    <motion.div className="pointer-events-none absolute -inset-10 overflow-hidden bg-[#1A1A1A]" style={{ y, scale }}>
